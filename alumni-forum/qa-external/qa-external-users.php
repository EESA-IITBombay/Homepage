...
*/
		...
		if ( isset($_SESSION['AUTH_TOKEN']) && isset($_SESSION['USER_NAME']) && isset($_SESSION['USER_ID']) ) {
			...
			$_USER=mysql_fetch_assoc(
				mysql_query(
					"SELECT auth_token,mail,user_level FROM tbl_students WHERE user_id='".mysql_real_escape_string($_SESSION['USER_ID'], $qa_db_connection)."'",
					$qa_db_connection
				)
			);
			...
			if (is_array($_USER)) {
				return array(
					...
					'publicusername' => $_SESSION['USER_NAME'],
					'email' => empty($_USER['mail'])?$_SESSION['USER_ID'].'@iitb.ac.in':$_USER['mail'],
					'level' => $_USER['user_level']
				);
			}
		}
		return null;
...
*/
	...
	//	Until you edit this function, always return null
               	...
               	$_USER=mysql_fetch_assoc(
                       	mysql_query(
                               	"SELECT ldap_id, mail FROM tbl_students WHERE user_id='".mysql_real_escape_string($userid, $qa_db_connection)."'",
                               	$qa_db_connection
                       	)
               	);

               	if (is_array($_USER))
                       	return empty($_USER['mail'])?$_USER['ldap_id'].'@iitb.ac.in':$_USER['mail'];
               	return null;
	/*
		Example 1 - suitable if:
	...
	//	Until you edit this function, always return null
               	...
               	if (count($publicusernames)) {
                        ...
                        $escapedusernames=array();
                       	...
                       	$results=mysql_query(
                               	'SELECT display_name, user_id FROM tbl_students WHERE display_name IN ('.implode(',', $escapedusernames).')',
                               	$qa_db_connection
                       	);

                       	while ($result=mysql_fetch_assoc($results))
                               	$publictouserid[$result['display_name']]=$result['user_id'];
               	}
...
*/
	...
	//	Until you edit this function, always return null
               	...
               	if (count($userids)) {
                       	...
                       	$results=mysql_query(
                               	'SELECT display_name, user_id FROM tbl_students WHERE user_id IN ('.implode(',', $escapeduserids).')',
                               	$qa_db_connection
                       	);

                       	while ($result=mysql_fetch_assoc($results))
                               	$useridtopublic[$result['user_id']]=$result['display_name'];
               	}
...
*/
	{
               	...
               	$_USER=mysql_fetch_assoc(
                       	mysql_query(
                               	"SELECT user_avatar FROM tbl_students WHERE user_id='".mysql_real_escape_string($userid, $qa_db_connection)."'",
                               	$qa_db_connection
                       	)
               	);
               	if (is_array($_USER) && !empty($_USER['user_avatar']))
			return "<img src='".$_USER['user_avatar']."' width='".$htmlsize."' height='".$htmlsize."' class='qa-avatar-image' alt=''/>";
